...
df_new.to_csv("proteins.csv", index=False)


df = pd.read_csv('proteins.csv', low_memory=False)
df["pert_id"] = df["pert_id"].astype(str)
nan_thres = 0.1
...
prot_only_diff = pd.DataFrame(
    (prot_prot_only.to_numpy() - control_prot_only.to_numpy()).astype(float),
...
# qc = expr_csv[expr_csv.columns.tolist()[-20:]]
# qc = qc.apply(get_first_non_zero_column, axis=1)
